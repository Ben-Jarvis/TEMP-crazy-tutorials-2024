...
class MotionPlanner3D():
    ...
    def __init__(self, start, obstacles, bounds, grid_size, goal):
        ...
        path_a_star = ast.find_path()

        self.trajectory_setpoints = None

        self.init_params(path_a_star)

        self.run_planner(obstacles, path_a_star)
    ...
    def run_planner(self, obs, path_waypoints):    
        ...
        trajectory_setpoints, time_setpoints = self.poly_setpoint_extraction(poly_coeffs)
        self.plot(obs, path_waypoints, trajectory_setpoints)

        self.trajectory_setpoints = trajectory_setpoints
    ...
    def init_params(self, path_waypoints):
        ...
        self.disc_steps = 4 #integer number oi steps to divide every path segment into to provide the reference positions for PID control # IDEAL: Between 10 and 20
        self.vel_lim = 5.0 # m/s
        self.acc_lim = 40.0 # m/s²

        t_f = 10 # Final time at the end of the path # As low as 4 or 5 (3.5)
        ...
        # self.times = [0]
        # [self.times.append(np.sum(dist_ratios[:i+1])*t_f) for i in range(len(dist_ratios))]
    ...
    def compute_poly_matrix(self, t):
        ...
        # Outputs: 
        # - The constraint matrix "A" [5 x 6]
        # The "A" matrix is used to solve the constraint_vector (b)  = A (constraint_factor_matrix) * poly_coeffs (polynomial_coefficients for one segment)
    ...
    def compute_poly_coefficients(self, path_waypoints):
        ...
        # - path_waypoints: The sequence of input path waypoints provided by the path-planner, including the start and final goal position: Vector of m waypoints, consisting of a tuple with three reference positions each as provided by AStar
        # Outputs:
        # - poly_coeffs: The polynomial coefficients for each segment of the path [6 x (m-1) x 3]
    ...
    def poly_setpoint_extraction(self, poly_coeffs):
        ...
        # Use the class features: self.disc_steps, self.times, self.poly_coeffs, self.vel_lim, self.acc_lim
        ...
        for i,t in enumerate(time_setpoints):
            ...
            x_vals[i,:] = np.dot(self.compute_poly_matrix(t-self.times[segment])[0],coeff_x[segment*6:(segment+1)*6])
            y_vals[i,:] = np.dot(self.compute_poly_matrix(t-self.times[segment])[0],coeff_y[segment*6:(segment+1)*6])
            z_vals[i,:] = np.dot(self.compute_poly_matrix(t-self.times[segment])[0],coeff_z[segment*6:(segment+1)*6])
            # Determine the x,y and z velocities at every reference time
            v_x_vals[i,:] = np.dot(self.compute_poly_matrix(t-self.times[segment])[1],coeff_x[segment*6:(segment+1)*6])
            v_y_vals[i,:] = np.dot(self.compute_poly_matrix(t-self.times[segment])[1],coeff_y[segment*6:(segment+1)*6])
            v_z_vals[i,:] = np.dot(self.compute_poly_matrix(t-self.times[segment])[1],coeff_z[segment*6:(segment+1)*6])
            # Determine the x,y and z accelerations at every reference time
            a_x_vals[i,:] = np.dot(self.compute_poly_matrix(t-self.times[segment])[2],coeff_x[segment*6:(segment+1)*6])
            a_y_vals[i,:] = np.dot(self.compute_poly_matrix(t-self.times[segment])[2],coeff_y[segment*6:(segment+1)*6])
            a_z_vals[i,:] = np.dot(self.compute_poly_matrix(t-self.times[segment])[2],coeff_z[segment*6:(segment+1)*6])
...
if __name__ == '__main__':
    ...
    mp_obj = MotionPlanner3D(start, obstacles, bounds, grid_size, goal)
